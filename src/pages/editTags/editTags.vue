<template>
  <view class="page">
    <view class="tag-item-container" v-for="(item, index) in tagsList" :key="index">
      ...
      <view class="tag-content flex" v-else>
        ...
        <view class="handle-container">
          <text class="margin-right text-red" @click="onDelete(item._id, item.name, index)">删除</text>
          <text class="margin-right text-grey" @click="onEdit(item._id)">编辑</text>
        </view>
    ...
    <view class="tag-item-container flex">
      ...
      <view class="action-btn text-sm bg-theme" @click="onSave">保存</view>
    </view>
  </view>
</template>
